...
functio with lexical envirnment called closure
...
example 2=>With argument
                    ...
                    person.print.call(person3 , "mumbai" , "india");
                    
example2=>  call printsAnimals such that its prints all animals in object
              const animals=[
                {specis:'tiger' ,name:'king'},
                {specis:'whale' ,name:'queen'}
            ];
...
ex2=>   appends array into another array without concate
...
ex2=> calculate max number from array
...
ex2=>
      function checkPass(success , fails){
      let passwors=prompt('passwors',"");
      if(passwors == 'aamirali') success();
      else fails();
  }
...
checkPass(user.loginSuccess.bind(user) , user.loginfails.bind(user));
